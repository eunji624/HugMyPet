export class ReviewService {
	...
	updateReview = async (reviewId, title, content, score) => {
		...
		//현재 펫시터의 평균평점 가져오기
		const getPetSitterScore = await this.reviewRepository.getPetSitterScore(+updateReview.petSitterId);
		const oldScore = getPetSitterScore.score;
		const scoreAvg = Math.round((score + oldScore) / 2);

		const modifyPetSitterScore = await this.reviewRepository.updatePetSitterScore(+updateReview.petSitterId, scoreAvg);
