...
export function drawThisMontDelteDatesCalendar() {
	...
	for (let i = 1; i <= nextDate; i++) {
		let newDate = i < 10 ? `0${i}` : i;
		const dayElement = $(
			`<div class="delete-day current" deleteId=${currentYear}-${currentMonth + 1}-${newDate}>` + i + '</div>'
		);
		if (today.getMonth() === currentMonth && i === today.getDate()) {
			dayElement.addClass('today');
		}
...
export function drawNextMonthDeleteDatesCalendar() {
	...
	for (let i = 1; i <= nextDate; i++) {
		let newDate = i < 10 ? `0${i}` : i;
		const dayElement = $(
			`<div class="next-delete-day current" deleteid=${nextMonthYear}-${newNextMonth}-${newDate}>` + i + '</div>'
		);
